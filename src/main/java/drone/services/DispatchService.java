...
public class DispatchService {
  ...
  public ResponseEntity<ApiResponse> registerDrone(
    DroneRequestPayload payLoad
  ) {
    Optional<Drone> drone = payLoad.getId() != null
      ? droneRepository.findById(payLoad.getId())
      : Optional.empty();

    //Drone payload validation
    if (payLoad.getSerialNumber() != null && payLoad.getModel() != null) {
      ...
      transportationDrone.setWeightLimit(
        payLoad.getWeightLimit() != null ? payLoad.getWeightLimit() : 500
      );
      transportationDrone.setBatteryCapacity(
        payLoad.getBatteryCapacity() != null
          ? payLoad.getBatteryCapacity()
          : 100
      );
      transportationDrone.setState(
        payLoad.getState() != null ? payLoad.getState() : DroneState.IDLE
      );

      if (droneRepository.save(transportationDrone) != null) {
        return new ResponseEntity<>(
          new ApiResponse(false, "Drone not Registered, Try Again"),
          HttpStatus.OK
        );
      }
    }
